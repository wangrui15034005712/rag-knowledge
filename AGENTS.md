...
2. ChromaDB 做 **向量相似度匹配**，返回 TOP_K=4 个文档块
...
**解决办法**：`config.py:11` 换非 thinking 模型，如 `qwen2.5-32b`、`qwen35-9b`。

## 文档导入

- 上传文档到 `docs/` 后，需点击"导入到知识库"按钮（手动触发）
- **不走大 LLM**，只走 embedding 模型（`bge-m3` / `octen-embedding-4b`）
- 增量去重：SHA256 hash 对比，内容无变化自动跳过
- 分块打印在导入日志中可见（前 2 个 chunk 预览）

## Debug 日志

`LoggingCallbackHandler` 打印：
- LLM 收到的完整 Prompt（包括检索到的文档块）
- 改写后的搜索查询
- 检索到的文档块内容
- 日志级别在 `config.py:28` 设置 `LOG_LEVEL`
