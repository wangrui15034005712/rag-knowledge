...
def load_translation_model():
    ...
    try:
        ...
        zh_en = next(
            (p for p in installed if p.from_code == "zh" and p.to_code == "en"),
            None
        )

        if zh_en is None:
            pkg_path = Path(model_dir) / "zh-en.argosmodel"
            if pkg_path.exists():
        ...
        return argostranslate.translate.get_translation_from_codes("zh", "en")
...
if "zh_input" not in st.session_state:
    st.session_state.zh_input = ""
if "en_output" not in st.session_state:
    st.session_state.en_output = ""
...
def do_translate():
    text = st.session_state.zh_input.strip()
    if not text:
        st.session_state.en_output = ""
        st.session_state.translate_time = 0
        return
    ...
    try:
        ...
        st.session_state.en_output = result
...
with col_input:
    ...
    zh_input = st.text_area(
        "中文原文",
        value=st.session_state.zh_input,
        height=370,
        placeholder="在此输入中文文本...\n\nEnter 发送 | Shift+Enter 换行",
        label_visibility="collapsed",
    )
    if zh_input != st.session_state.zh_input:
        st.session_state.zh_input = zh_input

    input_len = len(st.session_state.zh_input.strip())
    st.caption(f"字符数: {input_len}")

with col_buttons:
    st.markdown("<br><br>", unsafe_allow_html=True)

    if st.button("中->英", use_container_width=True, type="primary"):
        do_translate()
...
with col_output:
    ...
    st.text_area(
        "英文译文",
        value=st.session_state.en_output,
        height=370,
    ...
    output_len = len(st.session_state.en_output)
